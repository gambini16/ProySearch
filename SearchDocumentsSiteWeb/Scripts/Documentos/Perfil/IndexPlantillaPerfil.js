...
function Index() {
    $.ajax({
        ...
        success: function (data) {
            clearLoading();
            var table = $('#tblPerfilesPlantilla').DataTable();
            table.clear().draw();
...
$(document).ready(function () {
    ...
    /************************/
    // PERFILES
    $('#tblPerfilesPlantilla').DataTable({
    ...
    /* SELECCION REGISTRO */
    $("#tblPerfilesPlantilla tbody").on('click', 'tr', function (event) {
        event.preventDefault();
        $("#tblPerfilesPlantilla tbody tr").removeClass('row_selected');
    ...
    /*BOTÓN CONFIGURAR*/
    $('#tblPerfilesPlantilla tbody').on("dblclick", 'tr', function (event) {
        event.preventDefault();
        var $this = $(this);
        var row = $this.closest("tr");
        var intIdCodigoPerfil = row.find('td:first').text();
        var strNombrePerfil = row.find('td:nth-child(2)').text();
        var url = BASE_APP_URL + 'Perfil/PlantillaPerfil?$intIdCodigoPerfil=' + EncriptarParametro(intIdCodigoPerfil) + '&$strNombrePerfil=' + EncriptarParametro(strNombrePerfil);
        window.location.href = url;
    });
    /*BOTÓN CONFIGURAR*/
    $("#tblPerfilesPlantilla tbody").on('click', 'img.btnConfigurar', function (event) {
        event.preventDefault();
        var $this = $(this);
        var row = $this.closest("tr");
        var intIdCodigoPerfil = row.find('td:first').text();
